...
class Net(object):
	...
	def dfus_block(self, bottom, i):
		...
		with tf.variable_scope('dfus_block' + str(i), reuse=self.reuse):
			conv1  = act(conv2d(bottom, 128, [1, 1], wl=None, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_i'), name='relu' + str(i) + '_i')

			feat1  = act(conv2d(conv1, 32, [3, 3], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_1'), name='relu' + str(i) + '_1')
			feat15 = act(conv2d(feat1, 16, [1, 1], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_15'), name='relu' + str(i) + '_15')

			feat2  = act(conv2d(conv1, 32, [3, 3], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_2'), name='relu' + str(i) + '_2')
			feat23 = act(conv2d(feat2, 16, [1, 1], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_23'), name='relu' + str(i) + '_23')

			feat = tf.concat([feat1, feat15, feat2, feat23], 3, name='conv' + str(i) + '_c1')
			feat = act(conv2d(feat, 32, [1, 1], wl=None, reuse=self.reuse, tower_index=self.tower, scope='conv' + str(i) + '_r'), name='relu' + str(i) + '_r')
	...
	def ddfn(self, bottom, step, b=10):
		...
		with tf.variable_scope('ddfn_' + str(step), reuse=self.reuse):
			with tf.name_scope('msfeat'):
				conv13  = act(conv2d(bottom, 32, [3, 3], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv1_3'), name='relu1_3')
				conv15  = act(conv2d(bottom, 32, [3, 3], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv1_5'), name='relu1_5')

				conv135 = act(conv2d(conv13, 32, [1, 1], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv1_3_5'), name='relu1_3_5')
				conv153 = act(conv2d(conv15, 32, [1, 1], wl=self.wl, reuse=self.reuse, tower_index=self.tower, scope='conv1_5_3'), name='relu1_5_3')
